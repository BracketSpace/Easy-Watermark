...
class BulkActions extends Model {
	...
	init() {
		...
		this.bulkActionsSelectors
			...
			.val( -1 )
			.on( 'change', this.selectBulkAction );
	...
	prepare() {
		const action = this.get( 'action' ),
			...
			statusText =
				'watermark' === action
					? ew.i18n.watermarkingStatus
					: ew.i18n.restoringStatus,
			successMessage =
				'watermark' === action
					? ew.i18n.watermarkingSuccessMessage
					: ew.i18n.restoringSuccessMessage;
		...
		if ( ! selection.length ) {
			this.status().set( {
				successMessage:
					'watermark' === action
						? ew.i18n.watermarkingNoItems
						: ew.i18n.restoringNoItems,
			} );
	...
	doActionRecursive() {
		const bulkAction = this.get( 'action' ),
			...
			.done( ( response ) => {
				if ( true === response.success ) {
					if ( response.data.attachmentVersion ) {
						const img = this.form.find(
								'tr#post-' + data.attachment_id + ' img'
							),
							src = imageVersion(
								img.attr( 'src' ),
	...
	actionDone() {
		...
		if ( error ) {
			const row = this.form.find( 'tr#post-' + currentID ),
