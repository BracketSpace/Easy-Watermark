	...
	<h1><?php esc_html_e( 'Easy Watermark', 'easy-watermark' ); ?></h1>
		...
		<?php if ( count( $tabs ) ) : ?>
			...
			<?php foreach ( $tabs as $tab => $label ) : ?>
				<a class="nav-tab<?php echo ( $tab === $current_tab ) ? ' nav-tab-active' : null; ?>" data-tab="<?php echo esc_attr( $tab ); ?>" href="<?php echo esc_url( add_query_arg( [ 'tab' => $tab ], $base_url ) ); ?>"><?php echo esc_html( $label ); ?></a>
			<?php endforeach; ?>
			</h2>
		<?php endif; ?>
